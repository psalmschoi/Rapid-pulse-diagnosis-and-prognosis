...
for t_low in time_lower:
    for t_up in tqdm(time_upper, desc=f"r value (t_low: {t_low}): "):
        ...
        row_index, col_idx, r_value_list, anomal_error_rate_list, report_r_max, report_a_min = Grid_Search("0.1C", t_low, t_up, soc_size)
        
        col_index = np.array(col_idx).reshape(2,-1)

        fig = plt.figure(fig_num, figsize = (12, 6))
        ...
        os.makedirs(f"data/output/r_value/size({soc_size})", exist_ok = True)

        Save_DF = pd.DataFrame(save_y_df)
        Save_DF.to_csv(f"data/output/r_value/size({soc_size})/r_value optimizing t_lower({t_low}) t_upper({t_up}).csv")
