...
class Data() :
    ...
    def get_Peak(self, D = np.arange(4,1201,2)) :
        start_dir = os.getcwd()
    ...
    def get_DCIR(self, current):
        ...
        self.Result_DCIR = Process.DCIR_Data(File_name = file_name, SOC = self.SOC) / current
        os.chdir(start_dir)
        
            
    def get_dVdQ(self):
        start_dir = os.getcwd()
        data_dir = os.path.join(start_dir, 'data/input/discharge', self.path)
        os.chdir(data_dir)

        file_name = self.path + "_" + str(self.num) + ".csv"
        
        self.dVdQ = Process.Discharge_Data(File_name = file_name)
